<form action="javascript:void(0)" id="commonform" name="commonform" class="form-horizontal" method="POST"
    ...
    <input type="hidden" id="allowanceId" value="{{ $allowanceId ?? '' }}">

    <div class="form-floating form-floating-outline mb-4">
        <input type="text" class="form-control" name="allowance" id="allowance" maxlength="50"
            placeholder="Allowance" />
        <label for="allowance">Allowance<span style="color:red">*</span></label>
        <span class="text-danger" id="allowance-error"></span>
    </div>
...
<script>
    ...
    if (allowanceId > 0) {
        var Url = "{{ config('apiConstants.ALLOWANCE_URLS.ALLOWANCE_VIEW') }}";
        fnCallAjaxHttpGetEvent(Url, {
            allowanceId: allowanceId
        }, true, true, function(
            response) {
            if (response.status === 200 && response.data) {
                $("#allowance").val(response.data.allowances_name);
    ...
    $("#commonform").validate({
        ...
        submitHandler: function(form) {
            ...
            var postData = {
                ...
                allowances_name: $("#allowance").val(),
                is_active: $("#is_active").is(':checked'),
            };
            var storeRoleUrl = "{{ config('apiConstants.ALLOWANCE_URLS.ALLOWANCE_STORE') }}";
            var updateRoleUrl = "{{ config('apiConstants.ALLOWANCE_URLS.ALLOWANCE_UPDATE') }}";
            var url = allowanceId > 0 ? updateRoleUrl : storeRoleUrl;
