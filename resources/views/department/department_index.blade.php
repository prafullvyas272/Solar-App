            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if ($permissions['canAdd'])
                    <button id="btnAdd" type="submit" class="btn btn-primary waves-effect waves-light"
                        onClick="fnAddEdit(this, '{{ url('department/create') }}', 0, 'Add Department')">
                        <span class="tf-icons mdi mdi-plus">&nbsp;</span>Add Department
                    </button>
                @endif
            </div>
            <hr class="my-0">
    ...
    <script type="text/javascript">
        ...
        function initializeDataTable() {
            $("#grid").DataTable({
                ...
                ajax: {
                    url: "{{ config('apiConstants.DEPARTMENT_URLS.DEPARTMENT') }}",
                    type: "GET",
                    headers: {
                        ...
                        render: function(data, type, row) {
                            ...
                            html += "<li class='list-inline-item'>" +
                                GetEditDeleteButton({{ $permissions['canDelete'] }},
                                    ...
                                    "',fnDeleteRecord," +
                                    data + ",'" +
                                    '{{ config('apiConstants.DEPARTMENT_URLS.DEPARTMENT_DELETE') }}' +
                                    "','#grid')", "Delete") +
                                "</li>";
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if ({{ $permissions['canEdit'] }}) {
                                return `<a href="javascript:void(0);" onclick="fnAddEdit(this,'{{ url('department/create') }}',${row.id}, 'Edit Department')"
